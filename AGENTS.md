...
|--------|---------|-----------------|
| **dolt** | Read/write Dolt database tables | `.crush.json`, `.claude/settings.json` |
| **mcp-yfinance-10ks** | Fetch financial statements from Yahoo Finance | `.crush.json`, `.claude/settings.json` |
| **mcp-sec-10ks** | Fetch financial statements from SEC 10-K filings | `.crush.json`, `.claude/settings.json` |
...
│   ├── mcp-config.json                # (for reference only)
│   └── skills/
│       ├── analyze-financials/
│       │   ├── SKILL.md               # Full analyze-financials workflow
│       │   └── references/
│       │       ├── anomaly-rules.md   # SGA, balance sheet, restatement rules
│       │       └── company-notes.md   # Per-company patterns
│       └── insert-financials/
│           └── SKILL.md               # Full insert-financials workflow
├── .devcontainer/
│   ├── devcontainer.json              # Codespace config
│   └── post-create.sh                 # Auto-setup MCP servers on boot
├── .copilot/
│   └── mcp-config.json                # GitHub Copilot MCP config
├── .opencode/
...
4. Registers Claude and Codex MCP servers from `.claude/settings.json`
...
- **`.claude/settings.json`** — MCP server URLs for Claude Code
...
- [ ] Verify MCP servers are available: `.crush.json`, `.claude/settings.json` are checked in
